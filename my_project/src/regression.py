...
def main():
    ...
    pub_mapping_path = os.path.join(project_root, "data", "publisher_region_mapping.csv")
    dev_mapping_path = os.path.join(project_root, "data", "developer_region_mapping.csv")
    ...
    df_reviews['Region_Pub'] = df_reviews.get('Publisher', pd.Series()).map(pub_map).fillna('Other')
    df_reviews['Region_Dev'] = df_reviews.get('Developer', pd.Series()).map(dev_map).fillna('Other')
    ...
    categorical_cols = [c for c in ['Region_Dev', 'Type'] if c in df_model.columns]
    ...
    plt.title('Key Factors (Outliers Removed)', fontsize=16)
    plt.axvline(x=0, color='black')
    plt.tight_layout()
    
    plt.savefig(os.path.join(output_dir_figures, "fig5_regression_coefficients.png"), dpi=300)
